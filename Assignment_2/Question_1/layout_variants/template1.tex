...
% --- layout: formatting-only changes to reduce length -----------------------
\setlength{\textfloatsep}{10pt plus 2pt minus 2pt}   % space around top/bottom floats
\setlength{\intextsep}{10pt plus 2pt minus 2pt}      % space around [H] floats
\setlength{\abovecaptionskip}{4pt}
\setlength{\belowcaptionskip}{0pt}

\newcommand{\todo}[1]{\textcolor{red}{\textbf{[TO WRITE: #1]}}}

\title{MGT 595: Quantitative Investing \\ Problem Set 2 \\ \large (layout test: template1)}
...
\textit{Prompt:} We gave ChatGPT the spreadsheet and the following prompt.
...
\textit{What we had to fix or adjust:} ChatGPT's code was sound and its figure
was not. Its chart omits the portfolios built from the actual data, which the
question asks to be shown alongside the simulated clouds, and it draws one
combined scatter rather than the two separate mean--standard deviation plots
the question specifies. A single extreme draw also stretches its horizontal
axis far beyond the rest of the points, compressing everything of interest
into the left third of the panel. The code we wrote for part (d), which
produces Figure~\ref{fig:q1d}, uses two panels on shared axes, marks the
actual-data portfolios, adds the efficient frontier and fixes the axis limits
from percentiles so that a single draw cannot rescale the plot.

Three smaller differences from our own implementation. ChatGPT worked in
decimals while the spreadsheet is in percent, so its output is not directly
comparable to our other tables. It did not filter the Ken French missing-data
codes, which is harmless here because we checked that this file contains none.
And it re-factorized the covariance matrix on every one of the 1{,}000 draws
rather than reusing a Cholesky factor, which is correct but slower.

\textit{Did the LLM apply the simulated weights to the actual returns?} Yes.
Its evaluation function multiplies the actual return matrix by the simulated
weights, and its methodology note says so explicitly. We tested this rather
than taking it on trust. Jorion shows that every simulated portfolio must be
suboptimal once measured against the original mean vector and covariance
matrix, so we checked whether any simulated portfolio beat the true optimum.
None did, for either the MVP or the tangency portfolio. Code that evaluated
the weights on the simulated sample would fail this test constantly. Its
per-industry weight dispersion also matches our own implementation.
